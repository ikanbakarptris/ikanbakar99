...
function WhatsAppCta({ className = "", url, text }: { className?: string, url?: string, text?: string }) {
  return (
    <a
...
function MapsCta({ className = "", url, text }: { className?: string, url?: string, text?: string }) {
  return (
    <a
      href={url || SHOP.maps}
      target="_blank"
...
<div className="relative mx-auto max-w-5xl px-4 py-10 md:grid md:grid-cols-2 md:items-center md:gap-10 md:py-16">
          <AnimateIn>
            ...
            <h1 className="mt-4 text-4xl leading-[1.05] font-bold md:text-5xl">
              {heroTitle}
            </h1>
            ...
            <dl className="mt-4 grid grid-cols-3 gap-3 text-center">
              {heroStats.map((stat, idx) => (
                <div key={idx} className="rounded-xl bg-card border border-border border-b-4 border-b-muted-foreground/20 px-2 py-3 shadow-sm hover:-translate-y-1 hover:shadow-md hover:border-b-primary/40 transition-all duration-300">
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-xs md:text-sm text-foreground/80 mt-6 px-4 py-3 bg-card rounded-2xl border border-border/40 shadow-sm backdrop-blur-sm">
                  <div className="flex items-center gap-4">
                    {settings?.social_facebook && (
                      <a href={settings.social_facebook} target="_blank" rel="noopener noreferrer" className="transition-all duration-300 hover:-translate-y-1 hover:text-[#1877F2] hover:drop-shadow-[0_0_8px_rgba(24,119,242,0.8)] flex items-center justify-center p-1" aria-label="Facebook">
                        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/></svg>
                      </a>
                    )}
                    {settings?.social_instagram && (
                      <a href={settings.social_instagram} target="_blank" rel="noopener noreferrer" className="transition-all duration-300 hover:-translate-y-1 hover:text-[#F56040] hover:drop-shadow-[0_0_8px_rgba(245,96,64,0.8)] flex items-center justify-center p-1" aria-label="Instagram">
                    ...
                    {settings?.social_tiktok && (
                      <a href={settings.social_tiktok} target="_blank" rel="noopener noreferrer" className="transition-all duration-300 hover:-translate-y-1 hover:text-[#000000] dark:hover:text-[#FFFFFF] hover:drop-shadow-[0_0_8px_rgba(0,0,0,0.6)] dark:hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.8)] flex items-center justify-center p-1" aria-label="TikTok">
                        <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 2.78-1.15 5.54-3.33 7.65-2.43 2.3-6.19 3.02-9.28 1.7-2.61-1.1-4.48-3.52-4.9-6.31-.47-2.91.4-5.99 2.45-8.15 2.11-2.2 5.17-3.23 8.16-2.8v4.06c-1.3-.26-2.73-.24-3.95.27-1.4.58-2.47 1.84-2.81 3.32-.38 1.63.1 3.4 1.34 4.54 1.25 1.14 3.05 1.48 4.67 1 1.44-.45 2.5-1.74 2.75-3.27.08-.49.1-.99.1-1.49-.03-5.71-.02-11.43-.02-17.15z"/></svg>
                      </a>
                    )}
                    <a href={dynamicWaUrl} target="_blank" rel="noopener noreferrer" className="transition-all duration-300 hover:-translate-y-1 hover:text-[#25D366] hover:drop-shadow-[0_0_8px_rgba(37,211,102,0.8)] flex items-center justify-center p-1" aria-label="WhatsApp">
                      <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.888-.788-1.489-1.761-1.663-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 00-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" /></svg>
                      </a>
                    {settings?.contact_email && (
                      <>
                        <div className="h-4 w-px bg-border/60 mx-1" />
                        <a href={`mailto:${settings.contact_email}`} className="transition-all duration-300 hover:-translate-y-1 hover:text-[#EA4335] hover:drop-shadow-[0_0_8px_rgba(234,67,53,0.8)] flex items-center gap-2 truncate text-foreground font-medium group p-1">
                          <svg className="w-5 h-5 fill-muted-foreground group-hover:fill-[#EA4335] transition-colors" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
      ...
      <section className="relative bg-muted/20 py-16 bg-dot-pattern border-y border-border/40 shadow-[inset_0_0_40px_rgba(0,0,0,0.02)]" aria-labelledby="layanan">
        <div className="mx-auto max-w-5xl px-4">
          <h2 id="layanan" className="text-2xl font-bold md:text-3xl text-gradient text-center mb-2">
            {servicesTitle}
          </h2>
      ...
      <section className="mx-auto max-w-5xl px-4 py-10" aria-labelledby="kepercayaan">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-2xl border border-border bg-card p-5 text-center hover:shadow-md transition-shadow duration-300">
            <p className="font-display text-5xl font-bold bg-gradient-to-br from-yellow-500 to-amber-700 bg-clip-text text-transparent drop-shadow-sm">{Number(shopRating).toFixed(1)}</p>
      ...
      <footer className="border-t border-border py-8 text-center text-xs text-muted-foreground bg-muted/30">
        © <span suppressHydrationWarning>{new Date().getFullYear()}</span> {shopName} · {shopAddress}
      </footer>
